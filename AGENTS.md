...
# 科研工作区 Agent Bootstrap

本文件是 Codex 入口层，不是主规则库。真正的 agent 系统只在 `.agent/`。
...
- `.agent/` 是唯一权威源
- `AGENTS.md` 只保留 bootstrap，不再承载完整规则、画像镜像、handoff 或 workflow 正文
- `CLAUDE.md` 是 Claude Code 入口层，必须保留并保持 bootstrap-only
- `.github/copilot-instructions.md` 也必须保持同样的 bootstrap-only 形态
- 更新 agent 结构后，运行：
